...
// @version      1.0.0
// @description  サスケ内の電話番号を選択可能なリンク表示にし、クリック時だけ非公開 PhoneBridge Core へ発信要求を渡します。編集時のHTML混入防止にも対応。
...
(function () {
  ...
  function isEditTrigger(target) {
    ...
    return !!text && /(編集|edit|pencil|pen|鉛筆)/iu.test(text);
  }
  ...
  function renderPhoneLinks() {
    ...
    table.querySelectorAll('tbody tr').forEach(enhanceRowPhones);
  }
